...
CWAGENT_CONFIG=/sbo/data/scratch/weji/CWAgent_config_$SLURM_CLUSTER_NAME.json

if [ ! -f $CWAGENT_CONFIG ]; then
	echo "Create CWAGENT_CONFIG " $CWAGENT_CONFIG
	sed "s/\$CLUSTER_NAME/$SLURM_CLUSTER_NAME/g" /sbo/data/scratch/weji/CWAgent_config_tpl.json > $CWAGENT_CONFIG
fi	
