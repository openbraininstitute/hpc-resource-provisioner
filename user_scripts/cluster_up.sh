...
export USR_SSH_KEY_ARN=$(echo ${CLUSTER} | jq -r '.cluster.user_private_ssh_key_arn')
if [ $USR_SSH_KEY_ARN = "null" ]
then
	echo "Error: could not get the user SSH key arn"
else
	echo "Getting the user SSH key arn: ${USR_SSH_KEY_ARN}"
	aws secretsmanager get-secret-value --secret-id=${USR_SSH_KEY_ARN} | jq -r .SecretString >| ${SSH_KEY_FILE}
...
export ADMIN_SSH_KEY_ARN=$(echo ${CLUSTER} | jq -r '.cluster.admin_private_ssh_key_arn')
if [ $ADMIN_SSH_KEY_ARN = "null" ]
then
	echo "Error: could not get the admin SSH key arn"
else
	echo "Getting the admin SSH key arn: ${ADMIN_SSH_KEY_ARN}"
	aws secretsmanager get-secret-value --secret-id=${ADMIN_SSH_KEY_ARN} | jq -r .SecretString >| ${ADMIN_KEY_FILE}
