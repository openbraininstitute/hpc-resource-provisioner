...
def queryable(func):
    def wrapper(dest_func, _body, kwargs):
        ...
        print(f"In middleware wrapper: calling func {func} with dest_func {dest_func}, _body {_body} and kwargs {kwargs}")
...
def create_cluster(func, body, kwargs):
    wait = kwargs.pop("wait", False)
    print(f"In middleware.py: calling func {func} with kwargs {kwargs}")
    ret = func(**kwargs)
    if wait and not kwargs.get("dryrun"):
        cloud_formation = boto3.client("cloudformation")
